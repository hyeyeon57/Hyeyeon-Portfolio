...
export const ProjectForm: React.FC<ProjectFormProps> = ({ project, onSave, onCancel }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const projectData = {
      ...
      category: formData.category as 'new' | 'renewal' | 'proposal' | 'etc' | 'design',
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label className="block text-sm font-medium text-text-secondary mb-2">갤러리 이미지 (선택, 최대 70개)</label>
        <input
          type="file"
          accept="image/*"
          multiple
        ...
        />
        <p className="text-xs text-text-secondary mt-1">최대 70개의 이미지를 선택하세요 (JPG, PNG, etc.)</p>
